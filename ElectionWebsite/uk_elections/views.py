...
def parse_consts(df):
    ...
    for row in df.index:
        co = get_const_object(df.loc[row,'Name'])
        if not co:
...
def create_const_instances(df,row,c):

    if df['Predecessors'][row] != '':
        predecessors = df['Original Predecessors'][row] + '|' + df['Predecessors'][row]
    else:
        predecessors = df['Original Predecessors'][row]

    try:
        created = [ELECTION.objects.get(year=df['Created'][row])]
    except:
        created = [ELECTION.objects.get(year=fullElectionList[0])]

    if df['Re-created'][row] != '':
        created += [ELECTION.objects.get(year=x) for x in str(df['Re-created'][row]).split('|')]
    if df['Abolished'][row] != '' :
        abolished = [ELECTION.objects.get(year=x) for x in str(df['Abolished'][row]).split('|')]
    else:
        abolished = []

    for i in range(0,len(created)):
        if len(abolished) == 0 or i >= len(abolished):
            end_election = None
        else:
            end_election = ELECTION.objects.get(year=abolished[i])

        ci = CONSTINSTANCE(constituency=c,
                           created=ELECTION.objects.get(year=created[i]),
                           abolished=end_election,
                           predecessors=predecessors,
                           successors=df['Successors'][row])
        ci.save()

def create_const_seats(df,row,c,electionList):

    seatFields = {'4 MPs':4,'3 MPs':3,'2 MPs':2,'1 MP':1}
    seatDict = {}
    prevNames = []
    nameChanged = []
    if df['Previous Name 2'][row]:
        prevNames.append(df['Previous Name 2'][row])
        nameChanged.append(df['Changed 2'][row])
    if df['Previous Name'][row]:
        prevNames.append(df['Previous Name'][row])
        nameChanged.append(df['Changed'][row])

    for seatField in seatFields.keys():
        if df[seatField][row] == '':
            continue
        start, end = df[seatField][row].split('|')
        seatDict[start] = {'end':end,'seats':seatFields[seatField]}

    if prevNames and '|' not in nameChanged[-1]:
        current_name = prevNames.pop()
    elif prevNames and nameChanged[-1].split('|')[0] == electionList[0]:
        current_name = prevNames.pop()
    else:
        current_name = c.name
    current_seats = 1
    ...
    for election in allElections:
        if election in seatDict.keys():
            current_seats = seatDict[election]['seats']
            end_election = seatDict[election]['end']
        elif election == end_election:
            current_seats = 1
        if nameChanged and election == nameChanged[-1].split('|')[0]:
            if prevNames:
                current_name = prevNames.pop()
                if '|' in nameChanged[-1]:
                    name_end_election = nameChanged[-1].split('|')[1]
                nameChanged.pop()
            else:
                current_name = c.name
                nameChanged.pop()
        elif election == name_end_election:
            current_name = c.name
        if election not in electionList:
            continue

        try:
            CONSTSEATS.objects.get(election=ELECTION.objects.get(year=election), constituency=c)
        except:
            cs = CONSTSEATS(election=ELECTION.objects.get(year=election),
                            constituency=c,
                            name=current_name,
                            seats=current_seats)
            cs.save()

def create_const_seats_db(c):

    e_list = c.election_list.split(',')
    all_done = True
    for e in e_list:
        try:
            CONSTSEATS.objects.get(election=ELECTION.objects.get(year=e), constituency=c)
        except:
            all_done = False
            break
    ...
    seatFields = {'4 MPs':[c.four_mps,4],'3 MPs':[c.three_mps,3],'2 MPs':[c.two_mps,2]}
    seatDict = {}
    prevNames = []
    nameChanged = []

    if c.prev_name2:
        prevNames.append(c.prev_name2)
        nameChanged.append(c.name_changed2)
    if c.prev_name1:
        prevNames.append(c.prev_name1)
        nameChanged.append(c.name_changed1)

    for seatField in seatFields.keys():
        if seatFields[seatField][0] == '':
            continue
        start, end = seatFields[seatField][0].split('|')
        seatDict[start] = {'end':end,'seats':seatFields[seatField][1]}

    if prevNames and '|' not in nameChanged[-1]:
        current_name = prevNames.pop()
    elif prevNames and nameChanged[-1].split('|')[0] == e_list[0]:
        current_name = prevNames.pop()
    else:
        current_name = c.name
